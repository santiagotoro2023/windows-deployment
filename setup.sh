...
write_files() {
  ...
  mkdir -p "${DIR}"/{frontend,backend/data,ansible/{roles/{common,dc,fileserver,backupserver,rds_broker,rds_sessionhost,printserver,mgmt}/tasks,group_vars,inventory},docs}
...
.sb-search svg{position:absolute;left:7px;top:50%;transform:translateY(-50%);width:11px;height:11px;color:var(--text3);pointer-events:none;flex-shrink:0}
...
/* tree nodes */
.tr-section{padding:10px 10px 3px;font-size:9.5px;font-family:var(--mono);color:var(--text3);text-transform:uppercase;letter-spacing:.1em;font-weight:500}
...
.dp-scroll::-webkit-scrollbar{width:3px}#dp::-webkit-scrollbar-thumb{background:var(--b2)}
...
#log{background:#050810;border:1px solid var(--b1);border-radius:var(--rad);height:220px;overflow-y:auto;padding:9px 11px;font-family:var(--mono);font-size:10.5px;color:#4ade80;line-height:1.8;white-space:pre-wrap;word-break:break-all}
...
.tag{display:inline-flex;align-items:center;padding:1px 5px;border-radius:var(--rad);font-size:9px;font-family:var(--mono);font-weight:600;text-transform:uppercase;letter-spacing:.05em}
.empty{text-align:center;padding:36px 0;color:var(--text3)}.empty p{font-size:12px;margin-top:6px}
.role-badge{display:inline-flex;align-items:center;gap:4px;padding:2px 7px;border-radius:var(--rad);font-size:10.5px;font-weight:500;border:1px solid}
...
#modal{background:var(--panel);border:1px solid var(--b2);border-radius:var(--rad2);width:440px;max-height:82vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.7)}
...
<div id="sidebar">
  <div id="sb-top">
    <div class="sb-search">
      ...
      <input id="sb-input" placeholder="Filter…" oninput="renderTree(this.value.toLowerCase())">
    </div>
...
<div class="view" id="view-deploy">
  ...
  <div style="display:grid;grid-template-columns:1fr 280px;gap:14px">
    <div>
      ...
      <div class="sc">
        <div class="sc-head"><span style="font-family:var(--mono);color:var(--green);font-size:11.5px">$</span><h3>Ansible Output</h3><span id="live-b" style="margin-left:auto"></span></div>
        <div class="sc-body" style="padding:7px 9px"><div id="log">// Click ⚡ Deploy All to begin
// Each VM will be: cloned → static IP applied → RDP enabled → role features installed
</div></div>
      </div>
    </div>
    <div>
...
<div class="view" id="view-settings">
  ...
  <div class="sg">
    ...
    <div class="sg-body">
      <div class="g3">
        <div class="ff"><label>Network prefix</label><input id="s-net" value="192.168.1" placeholder="192.168.1"></div>
        <div class="ff"><label>Gateway</label><input id="s-gw" value="192.168.1.1" placeholder="192.168.1.1"></div>
        <div class="ff"><label>Subnet</label><input type="number" id="s-pfx" value="24" min="8" max="30"></div>
      </div>
      <div class="g2">
        <div class="ff"><label>Primary DNS</label><input id="s-dns1" value="8.8.8.8"></div>
        <div class="ff"><label>Secondary DNS</label><input id="s-dns2" value="8.8.4.4"></div>
      </div>
    </div>
  </div>
  <div class="sg">
    ...
    <div class="sg-body">
      <div class="g3">
        <div class="ff"><label>CPUs</label><input type="number" id="s-cpus" value="2" min="1" max="64"></div>
        <div class="ff"><label>RAM (MB)</label><input type="number" id="s-ram" value="4096" step="1024"></div>
        <div class="ff"><label>Disk (GB)</label><input type="number" id="s-disk" value="60" min="40"></div>
      </div>
      <div class="ff"><label>Windows Administrator Password</label><input type="password" id="s-pass" value="ChangeMe123!"></div>
    </div>
  </div>
  <div class="sg">
...
<!-- DETAIL PANEL -->
<div id="dp">
  <div class="dp-scroll"><div id="dp-body"></div></div>
</div>
</div><!-- /content -->
</div><!-- /layout -->
...
// ── Data model ──────────────────────────────────────────────────────────────
const ROLES = {
  dc:             { label:'Domain Controller',  icon:'🛡', color:'#f59e0b', bg:'rgba(245,158,11,.1)',  order:0, dcpu:2,  dram:4096,  ddisk:60  },
  fileserver:     { label:'File Server',        icon:'📁', color:'#5b9cf6', bg:'rgba(91,156,246,.1)',  order:1, dcpu:2,  dram:4096,  ddisk:200 },
  backupserver:   { label:'Backup Server',      icon:'💾', color:'#a78bfa', bg:'rgba(167,139,250,.1)',order:2, dcpu:2,  dram:4096,  ddisk:200 },
  rds_broker:     { label:'RDS Broker',         icon:'🔀', color:'#22d3ee', bg:'rgba(34,211,238,.1)', order:3, dcpu:2,  dram:4096,  ddisk:60  },
  rds_sessionhost:{ label:'RDS Session Host',   icon:'🖥', color:'#3ecf5d', bg:'rgba(62,207,93,.1)',  order:4, dcpu:4,  dram:8192,  ddisk:80  },
  printserver:    { label:'Print Server',       icon:'🖨', color:'#f97316', bg:'rgba(249,115,22,.1)', order:5, dcpu:2,  dram:2048,  ddisk:60  },
  mgmt:           { label:'Management',         icon:'⚙',  color:'#94a3b8', bg:'rgba(148,163,184,.1)',order:6, dcpu:2,  dram:4096,  ddisk:60  },
};
...
let S = {
  ...
  settings:{ net:'192.168.1', gw:'192.168.1.1', pfx:24, dns1:'8.8.8.8', dns2:'8.8.4.4', cpus:2, ram:4096, disk:60, pass:'ChangeMe123!', tz:'W. Europe Standard Time', locale:'de-CH' }
};
...
const persist = () => { try { localStorage.setItem('wd5', JSON.stringify({ hosts:S.hosts, vms:S.vms, settings:S.settings })); } catch(_){} };
const restore = () => { try { const d = JSON.parse(localStorage.getItem('wd5')||'{}'); if(d.hosts) S.hosts=d.hosts; if(d.vms) S.vms=d.vms; if(d.settings) Object.assign(S.settings, d.settings); } catch(_){} };
    ...
    return `<div class="vm-card ${ST_CLS[v.status]||'st-pend'}${S.selVm===v.id?' sel':''}" onclick="clickVm('${v.id}')">
      <div class="vc-top">
        ...
        <div style="flex:1;min-width:0">
          <div class="vc-name">${v.hostname}</div>
          <div class="vc-ip">${v.ip}</div>
        </div>
...
function showVmDetail(id) {
  ...
  $('dp-body').innerHTML = `
    ...
    <div class="dp-sec">
      ...
      <div class="dp-row"><span class="dp-k">Timezone</span><span class="dp-v" style="font-size:10px">${S.settings.tz.replace(' Standard Time','')}</span></div>
    </div>
    <div class="dp-actions">
      <button class="btn btn-g btn-fw" onclick="openModal('edit','${v.id}')">✏ Edit</button>
      <div class="sep"></div>
...
const LOG_LINES = [
  '[windows-deployment] Starting deployment…',
  '[proxmox] API auth OK',
  '[proxmox] Cloning VMID 9000 → dc01',
  '[proxmox] Cloud-Init: dc01 ip=192.168.1.10/24 gw=192.168.1.1 dns=8.8.8.8',
  '[proxmox] Starting dc01…',
  '[ansible] Waiting for WinRM 192.168.1.10:5985…',
  '[ansible] WinRM connected ✓',
  '[ansible] TASK Set-hostname dc01 [changed]',
  '[ansible] TASK Set-timezone W. Europe Standard Time [ok]',
  '[ansible] TASK Set-DNS 8.8.8.8 8.8.4.4 [ok]',
  '[ansible] TASK Enable-RDP [ok]',
  '[ansible] TASK Allow-RDP firewall [ok]',
  '[ansible] TASK Install AD-Domain-Services [ok]',
  '[ansible] TASK Install DNS [ok]',
  '[ansible] TASK Install DHCP [ok]',
  '[ansible] TASK Install GPMC (Group Policy) [ok]',
  '[ansible] TASK Install RSAT-AD-Tools [ok]',
  '[proxmox] Cloning VMID 9000 → rds-broker01',
  '[proxmox] Cloud-Init: rds-broker01 ip=192.168.1.20/24',
  '[ansible] TASK Enable-RDP [ok]',
  '[ansible] TASK Install RDS-Connection-Broker [ok]',
  '[ansible] TASK Install RDS-Licensing [ok]',
  '[ansible] TASK Install RDS-Web-Access [ok]',
  '[proxmox] Cloning VMID 9000 → rds-sh01',
  '[ansible] TASK Install RDS-RD-Server [ok]',
  '[ansible] TASK Install Desktop-Experience [ok]',
  '[windows-deployment] ✓ All roles complete',
];
let _di = null;
function startDeploy() {
  if (S.deploying) return; S.deploying = true; $('dep-btn').disabled = true;
  $('live-b').innerHTML = '<span style="font-size:10px;color:var(--green);font-family:var(--mono);animation:blink .8s infinite">● LIVE</span>';
  const log = $('log'); log.textContent = ''; let i = 0;
  _di = setInterval(() => {
    if (i >= LOG_LINES.length) {
      clearInterval(_di); S.deploying = false; $('dep-btn').disabled = false;
      $('live-b').innerHTML = '<span style="font-size:10px;color:var(--green);font-family:var(--mono)">✓ Done</span>';
      return;
    }
    log.textContent += LOG_LINES[i] + '\n'; log.scrollTop = log.scrollHeight; i++;
  }, 320);
}
...
function saveSettings() {
  const s = S.settings;
  s.net=$('s-net').value; s.gw=$('s-gw').value; s.pfx=+$('s-pfx').value||24;
  s.dns1=$('s-dns1').value; s.dns2=$('s-dns2').value;
  s.cpus=+$('s-cpus').value||2; s.ram=+$('s-ram').value||4096; s.disk=+$('s-disk').value||60;
  s.pass=$('s-pass').value; s.tz=$('s-tz').value; s.locale=$('s-locale').value;
  persist();
...
function openModal(type, id) {
  ...
  if (type === 'host') {
    ...
    $('modal-body').innerHTML = `
      <div class="ff"><label>Display Name</label><input id="m-name" placeholder="pve-main"></div>
      <div class="g2">
        <div class="ff"><label>Host IP / FQDN</label><input id="m-host" placeholder="192.168.1.2"></div>
        <div class="ff"><label>Node Name</label><input id="m-node" value="pve"></div>
      </div>
      <div class="ff"><label>API Token ID</label><input id="m-tokid" placeholder="root@pam!windeployment"></div>
      <div class="ff"><label>API Token Secret</label><input type="password" id="m-toksec" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"></div>
      <div class="g2">
        <div class="ff"><label>Template VMID</label><input type="number" id="m-tmpl" value="9000"></div>
        <div class="ff"><label>Storage Pool</label><input id="m-stor" value="local-lvm"></div>
      </div>
      <div class="ff"><label>Network Bridge</label><input id="m-bridge" value="vmbr0"></div>`;
  ...
  if (type === 'vm') {
    ...
    $('modal-body').innerHTML = `
      ...
      <div class="g2">
        <div class="ff"><label>Hostname</label><input id="m-hn" placeholder="dc01"></div>
        <div class="ff"><label>IP Address</label><input id="m-ip" placeholder="${s.net}.10"></div>
      </div>
      <div class="g3">
        <div class="ff"><label>CPUs</label><input type="number" id="m-cpus" value="${r.dcpu}"></div>
        <div class="ff"><label>RAM (MB)</label><input type="number" id="m-ram" value="${r.dram}" step="1024"></div>
        <div class="ff"><label>Disk (GB)</label><input type="number" id="m-disk" value="${r.ddisk}"></div>
      </div>
      ${S.hosts.length > 1
  ...
  if (type === 'edit') {
    ...
    $('modal-body').innerHTML = `
      ...
      <div class="g2">
        <div class="ff"><label>Hostname</label><input id="e-hn" value="${v.hostname}"></div>
        <div class="ff"><label>IP Address</label><input id="e-ip" value="${v.ip}"></div>
      </div>
      <div class="g3">
        <div class="ff"><label>CPUs</label><input type="number" id="e-cpus" value="${v.cpus}"></div>
        <div class="ff"><label>RAM (MB)</label><input type="number" id="e-ram" value="${v.ram}" step="1024"></div>
        <div class="ff"><label>Disk (GB)</label><input type="number" id="e-disk" value="${v.disk}"></div>
      </div>
      ${S.hosts.length > 1
        ...
        : ''}`;
    $('modal-foot').innerHTML = `<button class="btn btn-g" onclick="closeModal()">Cancel</button><button class="btn btn-a" onclick="saveEdit('${id}')">Save</button>`;
  }
}
...
function applyRoleDef(role) {
  ...
  if (r.ddisk) $('m-disk').value = r.ddisk;
}

function saveHost() {
  const n=($('m-name').value||'').trim(), h=($('m-host').value||'').trim();
  if (!n||!h) { alert('Name and host IP required'); return; }
  S.hosts.push({ id:uid(), name:n, host:h, node:$('m-node').value||'pve', templateVmId:+$('m-tmpl').value||9000, storage:$('m-stor').value||'local-lvm', bridge:$('m-bridge').value||'vmbr0', _open:true });
  persist(); closeModal(); renderAll();
}

function saveVm() {
  const hn=($('m-hn').value||'').trim(), ip=($('m-ip').value||'').trim();
  if (!hn||!ip) { alert('Hostname and IP required'); return; }
  if (!S.hosts.length) { alert('Add a Proxmox host first'); return; }
  S.vms.push({ id:uid(), hostId:$('m-hid').value||S.hosts[0].id, role:$('m-role').value, hostname:hn, ip, cpus:+$('m-cpus').value||2, ram:+$('m-ram').value||4096, disk:+$('m-disk').value||60, status:'pending', prog:0 });
  persist(); closeModal(); renderAll();
}
...
function saveEdit(id) {
  const v = vb(id); if (!v) return;
  v.role = $('e-role').value;
  v.hostname = $('e-hn').value; v.ip = $('e-ip').value;
  v.cpus = +$('e-cpus').value||v.cpus; v.ram = +$('e-ram').value||v.ram; v.disk = +$('e-disk').value||v.disk;
...
function delHost(id) {
  ...
  S.hosts = S.hosts.filter(h => h.id !== id); S.vms = S.vms.filter(v => v.hostId !== id);
  persist(); closeDetail(); renderAll();
}
function closeModal() { $('modal-bg').classList.remove('open'); }
function renderAll() { renderTree(q()); renderOverview(); if (S.view==='deploy') renderDeploy(); if (S.selVm) showVmDetail(S.selVm); else if (S.selHost) showHostDetail(S.selHost); }
...
const express=require('express'),cors=require('cors'),fs=require('fs'),path=require('path'),https=require('https'),{exec}=require('child_process');
...
function pve(host,tid,ts,p){
  return new Promise((res,rej)=>{
    const req=https.request({hostname:host,port:8006,path:`/api2/json${p}`,method:'GET',headers:{'Authorization':`PVEAPIToken=${tid}=${ts}`},rejectUnauthorized:false},r=>{
...
// Hosts
app.get('/api/hosts',(req,res)=>res.json(load().hosts.map(h=>({...h,tokenSecret:'***'}))));
app.post('/api/hosts',async(req,res)=>{
  const{name,host,node,tokenId,tokenSecret,templateVmId,storage,bridge}=req.body;
  try{const t=await pve(host,tokenId,tokenSecret,`/nodes/${node}/status`);if(!t.data)throw new Error('No data');}
  catch(e){return res.status(400).json({error:`Cannot connect: ${e.message}`});}
  const c=load();c.hosts=c.hosts.filter(h=>h.host!==host);
  c.hosts.push({id:Date.now().toString(),name,host,node,tokenId,tokenSecret,templateVmId:+templateVmId||9000,storage:storage||'local-lvm',bridge:bridge||'vmbr0'});
  save(c);res.json({success:true});
});
app.delete('/api/hosts/:id',(req,res)=>{const c=load();c.hosts=c.hosts.filter(h=>h.id!==req.params.id);save(c);res.json({success:true});});

// VMs
app.get('/api/vms',(req,res)=>res.json(load().vms||[]));
app.post('/api/vms',(req,res)=>{const c=load();const vm={id:Date.now().toString(),...req.body,status:'pending',prog:0};c.vms=[...c.vms||[],vm];save(c);res.json({success:true,vm});});
app.put('/api/vms/:id',(req,res)=>{const c=load();const i=c.vms.findIndex(v=>v.id===req.params.id);if(i===-1)return res.status(404).json({error:'Not found'});c.vms[i]={...c.vms[i],...req.body,id:req.params.id};save(c);res.json({success:true});});
app.delete('/api/vms/:id',(req,res)=>{const c=load();c.vms=c.vms.filter(v=>v.id!==req.params.id);save(c);res.json({success:true});});
...
// Deploy
let running=false,dlog=[];
app.post('/api/deploy',(req,res)=>{
  if(running)return res.status(409).json({error:'Already running'});
  const c=load();
  if(!c.hosts?.length)return res.status(400).json({error:'No hosts configured'});
  ...
  let ini=`# Generated by windows-deployment\n[windows:children]\n${ROLE_ORDER.join('\n')}\n\n`;
  ROLE_ORDER.forEach(role=>{
    const m=c.vms.filter(v=>v.role===role);if(!m.length)return;
    ini+=`[${role}]\n`;m.forEach(v=>ini+=`${v.hostname} ansible_host=${v.ip}\n`);ini+='\n';
  });
  ini+=`[windows:vars]
...
`;
  ...
  const h=c.hosts[0];
  const ev=`proxmox_host=${h.host} proxmox_node=${h.node} proxmox_template_vmid=${h.templateVmId} proxmox_storage=${h.storage} proxmox_token_id=${h.tokenId} proxmox_token_secret=${h.tokenSecret} win_admin_pass="${s.pass||'ChangeMe123!'}"`;
  dlog=[];running=true;res.json({success:true});
  const p=exec(`cd "${ADIR}" && ansible-playbook site.yml -i inventory/hosts.ini --extra-vars '${ev}' 2>&1`);
  p.stdout?.on('data',d=>dlog.push(d));p.stderr?.on('data',d=>dlog.push(d));
  p.on('close',code=>{running=false;dlog.push(`\n[windows-deployment] Finished (exit ${code})`);});
});
app.get('/api/deploy/status',(req,res)=>res.json({running,log:dlog.join('')}));
...
# Overridden at runtime by the WebApp via --extra-vars
ansible_user: Administrator
ansible_connection: winrm
ansible_winrm_transport: basic
ansible_winrm_port: 5985
...
EOF
  ...
  # ---------------------------------------------------------------------------
  # ROLE: common
  # Applied to every VM: wait for WinRM, hostname, timezone, DNS, enable RDP
  # ---------------------------------------------------------------------------
...
EOF
  ...
  # ---------------------------------------------------------------------------
  # ROLE: dc
  # Installs: AD DS, DNS, DHCP, Group Policy (GPMC), RSAT tools
  # ---------------------------------------------------------------------------
...
- name: Install Domain Controller features
  ansible.windows.win_feature:
    name:
      ...
      - RSAT-DHCP             # DHCP management tools
...
EOF
  ...
  # ---------------------------------------------------------------------------
  # ROLE: fileserver
  # Installs: File Services, DFS Namespace + Replication, Resource Manager
  # ---------------------------------------------------------------------------
...
- name: Install File Server features
  ansible.windows.win_feature:
    name:
      - FS-FileServer          # Basic file server
...
EOF
  ...
  # ---------------------------------------------------------------------------
  # ROLE: backupserver
  # Blank server — no additional features installed
  # ---------------------------------------------------------------------------
  cat > "${DIR}/ansible/roles/backupserver/tasks/main.yml" << 'EOF'
---
# Backup Server — intentionally blank
# Common role already applied: hostname, timezone, DNS, RDP
# Install your backup software manually (e.g. Veeam B&R)
# Veeam Community Edition (free, ≤10 workloads): https://www.veeam.com
- name: Backup server ready
  ansible.builtin.debug:
    msg: "{{ inventory_hostname }} is ready. Install backup software manually."
EOF

  # ---------------------------------------------------------------------------
  # ROLE: rds_broker
  # Installs: RDS Connection Broker, RDS Licensing, RDS Web Access
  # ---------------------------------------------------------------------------
...
- name: Install RDS Broker features
  ansible.windows.win_feature:
    name:
      ...
      - RDS-Licensing          # RDS Licensing Server
      - RDS-Web-Access         # RDS Web Access portal
      - RSAT-RDS-Tools         # RDS management tools
    include_management_tools: yes
    state: present
...
EOF
  ...
  # Installs: RDS Session Host, Desktop Experience
  # ---------------------------------------------------------------------------
...
- name: Install RDS Session Host features
  ansible.windows.win_feature:
    name:
      ...
      - Desktop-Experience     # Desktop Experience (required for full RDS)
    include_management_tools: yes
    state: present
  register: sh_feature

- name: Reboot after Session Host install (required)
  ansible.windows.win_reboot:
    reboot_timeout: 600
...
EOF
  ...
  # ---------------------------------------------------------------------------
  # ROLE: printserver
  # Installs: Print Server, Print Services
  # ---------------------------------------------------------------------------
...
- name: Install Print Server features
  ansible.windows.win_feature:
    name:
      ...
      - RSAT-Print-Services    # Print management tools
    include_management_tools: yes
    state: present
...
EOF
  ...
  # ---------------------------------------------------------------------------
  # ROLE: mgmt
  # Installs: Full RSAT suite for central management
  # ---------------------------------------------------------------------------
...
- name: Install full RSAT management suite
  ansible.windows.win_feature:
    name:
      - RSAT                   # Remote Server Administration Tools (meta)
      - RSAT-AD-Tools          # Active Directory tools
      - RSAT-AD-PowerShell     # AD PowerShell
      - RSAT-DNS-Server        # DNS management
      - RSAT-DHCP              # DHCP management
      - RSAT-DFS-Mgmt-Con      # DFS management
      - RSAT-Print-Services    # Print management
      - RSAT-RDS-Tools         # RDS management
...
EOF
  ...
  # Clones template, sets CPU/RAM, applies Cloud-Init, starts VMs
  # ---------------------------------------------------------------------------
...
- name: Clone template VM for each server
...
- name: Apply Cloud-Init (static IP, gateway, DNS, admin password)
  ansible.builtin.shell: |
    VMID=$(pvesh get /nodes/{{ proxmox_node }}/qemu --output-format json | \
      python3 -c "import sys,json; print(next(v['vmid'] for v in json.load(sys.stdin) if v['name']=='{{ item.hostname }}'))")
    qm set $VMID \
...
- name: Wait 90 seconds for Cloud-Init and Windows boot
...
SVCEOF
  ...
  echo -e "  ${BOLD}Template:${NC}  See TEMPLATE.md\n"
}
...
status() {
  ...
  echo ""
  journalctl -u "${APP}" -n 20 --no-pager 2>/dev/null
}
